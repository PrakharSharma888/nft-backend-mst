...
const { ethers } = require('ethers')
 ...
 function createApp() {
  ...
  const MINT_ABI = ['function mintWorkshopCertificate(uint256,string,string,string) external returns (uint256)']
  ...
  app.post('/api/chain/prepare-mint', async (req, res) => {
    try {
      ...
      if (!rpc) {
        return res
          .status(500)
          .json({ ok: false, reason: 'Missing VITE_RPC_URL (or MST_RPC_URL) in .env for server-side mint checks.' })
      }

      const { contractAddress, from, eventId, studentName, branch, tokenURI } = req.body || {}
      if (
        !contractAddress ||
        !from ||
        eventId === undefined ||
        ...
        return res.status(400).json({ ok: false, reason: 'Missing contractAddress/from/eventId/studentName/branch/tokenURI' })
      }
      ...
      const args = [BigInt(eventId), String(studentName).trim(), String(branch).trim(), String(tokenURI).trim()]
